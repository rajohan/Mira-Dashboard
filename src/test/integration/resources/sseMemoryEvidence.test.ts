...
const cgroupPath = expectedSseMemoryUnitCgroupPath(currentIntegrationUserId(), unitName);
const cgroupPolicy = sseMemoryEvidencePolicy.cgroup;
const scenarioPolicy = sseMemoryEvidencePolicy.scenario;
...
const eventsPerRound = integrationEventLimits.maximumSubscriberQueueEvents + 2;
...
describe("SSE memory evidence validation", () => {
    ...
    test("strictly reparses and revalidates child evidence", () => {
        ...
        expect(parseSseMemoryEvidence(JSON.stringify(evidence))).toEqual(evidence);
        expect(() => parseSseMemoryEvidence('{"verdict":"VALIDATED"}')).toThrow();

        const unknownField = { ...evidence, unexpected: true };
        expect(() => parseSseMemoryEvidence(JSON.stringify(unknownField))).toThrow();

        const tamperedInvariant = {
            ...evidence,
            feed: { ...evidence.feed, activeSubscribers: 1 },
        };
        expect(() => parseSseMemoryEvidence(JSON.stringify(tamperedInvariant))).toThrow(
            "Active subscribers after cleanup"
        );
        ...
        expect(() => parseSseMemoryEvidence(JSON.stringify(invalidRevision))).toThrow(
            "Bun revision is not a full commit SHA"
        );
        ...
        expect(() => parseSseMemoryEvidence(JSON.stringify(tamperedDelta))).toThrow(
            "invalid high"
        );
    });
});
