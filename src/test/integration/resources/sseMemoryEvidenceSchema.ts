...
import type { RuntimeIdentity } from "../runtime/runtimeCandidate.ts";
import type { IntegrationEventFeedMetrics } from "../transport/realtime/eventFeed.ts";
...
export interface SseMemoryEvidenceCandidate {
    ...
    durationMs: number;
    feed: Readonly<IntegrationEventFeedMetrics>;
    process: {
...
const sseMemoryEvidenceSchema = v.strictObject({
 ...
 */
export function parseSerializedSseMemoryEvidence(value: string): SseMemoryEvidence {
    return v.parse(sseMemoryEvidenceSchema, JSON.parse(value) as unknown);
}
