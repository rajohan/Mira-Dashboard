...
describe("SSE memory evidence resource policy", () => {
    test("exports the reviewed immutable cgroup and scenario boundaries", () => {
        expect(sseMemoryEvidencePolicy).toEqual({
            cgroup: {
                cpuQuotaPercent: 50,
        ...
        expect(Object.isFrozen(sseMemoryEvidencePolicy)).toBeTrue();
        expect(Object.isFrozen(sseMemoryEvidencePolicy.cgroup)).toBeTrue();
        expect(Object.isFrozen(sseMemoryEvidencePolicy.scenario)).toBeTrue();
    });
