...
const reviewedBareBunImportSignatures: ReadonlyMap<string, string> = new Map([
    ["scripts/developmentFrontend.ts", "type:Server"],
...
function isInternalBareSpecifier(specifier: string): boolean {
    return /^(?:backend|frontend|qualification|scripts|src)\//u.test(specifier);
}
...
export function validateExternalImport(
    ...
    if (sourceImport.kind === "module-loader" && specifier === undefined) {
        return violation(
            importer,
            sourceImport,
    ...
    if (
        (importerRole === "browser" || importerRole === "browser-app") &&
