...
export { isTestPath, legacyScriptImportAllowlist } from "./sourceTopologyPolicy.ts";
...
export function validateSourceFile(
    ...
    if (normalizedImporter.endsWith(".d.ts")) {
        return {
            importer: normalizedImporter,
            line: 1,
            message:
...
/**
 * Identifies a script edge into the legacy tree for exact allowlist accounting.
 * @param importer Repository-relative importing file.
 * @param sourceImport Parsed import or re-export.
 * @returns Stable allowlist key, or `undefined` for a non-legacy edge.
 */
export function legacyScriptImportKey(
    importer: string,
    sourceImport: SourceImport
): string | undefined {
    const normalizedImporter = normalizeRepositoryPath(importer);
    ...
    if (
        sourceRole(normalizedImporter) !== "scripts" ||
        specifier === undefined ||
    ...
    return targetRole === "legacy-backend" || targetRole === "legacy-frontend"
        ? legacyEdge(normalizedImporter, target)
        : undefined;
}
...
export function validateSourceImport(
    ...
    const normalizedImporter = normalizeRepositoryPath(importer);
    const importerRole = sourceRole(normalizedImporter);
    if (importerRole === "test") return undefined;
    ...
    if (!reviewedRelativeExtensions.has(targetExtension)) {
        return violation(
            normalizedImporter,
            sourceImport,
    ...
    if (targetRole === "environment-source") {
        return environmentSourceConsumers.has(normalizedImporter)
            ? undefined
            : violation(
                  normalizedImporter,
                  sourceImport,
    ...
    if (
        importerRole === "scripts" &&
        (targetRole === "legacy-backend" || targetRole === "legacy-frontend")
    ) {
        return legacyScriptImportAllowlist.has(legacyEdge(normalizedImporter, target))
            ? undefined
            : violation(
                  normalizedImporter,
                  sourceImport,
                  "New script imports into the legacy backend or frontend are forbidden"
              );
    }
