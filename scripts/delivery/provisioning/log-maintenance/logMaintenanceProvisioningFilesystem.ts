...
export type VerifiedLogMaintenanceProvisioningFile =
    LogMaintenanceProvisioningArtifactPolicy & {
        readonly bytes: Uint8Array;
        readonly sha256: string;
    };

/** Deterministic race boundaries used only by adversarial filesystem tests. */
export interface LogMaintenanceProvisioningFilesystemTestHooks {
...
interface StagedArtifact {
    ...
    readonly file: VerifiedLogMaintenanceProvisioningFile;
...
function validateFiles(files: readonly VerifiedLogMaintenanceProvisioningFile[]): void {
    if (
        files.length !== logMaintenanceProvisioningArtifacts.length ||
        files.some((file, index) => {
            const expected = logMaintenanceProvisioningArtifacts[index];
            return (
                expected === undefined ||
...
async function readExactFile(
    anchoredPath: string,
    expected: VerifiedLogMaintenanceProvisioningFile,
    directory: OpenedDirectory
): Promise<void> {
    let handle: FileHandle | undefined;
    let failed = false;
...
async function stageArtifact(
    directory: OpenedDirectory,
    destination: string,
    file: VerifiedLogMaintenanceProvisioningFile
    ...
    const temporary = path.join(
        descriptorRoot,
        `.mira-log-maintenance.${Bun.randomUUIDv7()}.tmp`
    );
...
async function openDestinationDirectories(
    ...
    files: readonly VerifiedLogMaintenanceProvisioningFile[],
    userId: number,
    groupId: number
): Promise<{
 ...
 */
export async function installLogMaintenanceProvisioningFiles(
    destinationRoot: string,
    files: readonly VerifiedLogMaintenanceProvisioningFile[],
    testHooks: LogMaintenanceProvisioningFilesystemTestHooks = {}
): Promise<void> {
    if (
