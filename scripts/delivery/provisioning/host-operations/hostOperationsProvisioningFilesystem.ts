...
function validOwnedDirectory(status: BigIntStats, userId: number): boolean {
    // System-owned policy directories may use a read-only service group (for
    // example root:polkitd 0750). Ownership and write authority remain rooted
    // in the expected UID; the directory group must not gain write access.
    return (
        status.isDirectory() &&
