...
import {
    openShutdownQualificationDatabase,
...
type QualificationServiceProcess = Bun.Subprocess<"ignore", "ignore", "ignore">;
...
export class CompleteShutdownQualificationError extends Data.TaggedError(
...
export interface CompleteShutdownQualificationReport {
...
export interface InterruptedShutdownQualificationReport {
...
function temporaryWorkspace() {
    return Effect.acquireRelease(
        Effect.tryPromise({
            ...
            try: () => mkdtemp(path.join(tmpdir(), "mira-shutdown-qualification-")),
        }),
        (workspacePath) =>
            Effect.tryPromise(() =>
...
function writeMarker(
    markerPath: string
): Effect.Effect<void, CompleteShutdownQualificationError> {
...
function awaitServiceExit(
    ...
    number,
    CompleteShutdownDeadlineError | CompleteShutdownQualificationError
> {
    return withDeadline(
        Effect.tryPromise({
            catch: (cause) =>
...
): Effect.Effect<readonly number[], CompleteShutdownQualificationError | E> {
...
export function readLinuxProcessGroupMembers(
    processGroupId: number
): Effect.Effect<readonly number[], CompleteShutdownQualificationError> {
    return Effect.gen(function* () {
...
function databaseSnapshotResource(databasePath: string) {
    return Effect.acquireRelease(
        Effect.sync(() => openShutdownQualificationDatabase(databasePath)),
        (database) => Effect.sync(() => database.close(true))
    );
}

/** Runs two production-shaped process generations against one WAL database. */
export const completeShutdownQualification = Effect.scoped(
    Effect.gen(function* () {
...
export const interruptedShutdownQualification = Effect.scoped(
    Effect.gen(function* () {
        const workspacePath = yield* temporaryWorkspace();
