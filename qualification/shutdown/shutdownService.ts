...
class ShutdownQualificationArgumentError extends Data.TaggedError(
    "ShutdownQualificationArgumentError"
)<{
...
function parseCommand(arguments_: readonly string[]): ServiceCommand {
    ...
    try {
        ...
        throw new ShutdownQualificationArgumentError({
            message: "Invalid shutdown qualification service arguments",
        });
    }
}
...
function databaseResource(databasePath: string) {
    return Effect.acquireRelease(
        Effect.try({
            ...
            try: () => openShutdownQualificationDatabase(databasePath),
        }),
    ...
    return Effect.acquireRelease(
        Effect.sync(() => {
            ...
            if (row?.generation !== generation) {
                statement.finalize();
                throw new Error("Shutdown qualification prepared statement failed");
            }
            return statement;
        }),
...
function runService(command: ServiceCommand) {
    ...
    const snapshot = (phase: ShutdownServiceStatus["phase"]): ShutdownServiceStatus => {
        if (application === undefined) {
            throw new Error("Shutdown qualification application is unavailable");
        }
