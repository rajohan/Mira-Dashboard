...
export const oversizedQualificationMessageBytes = 64 * 1024 + 1;
...
function parseHeaderLines(headerBytes: Buffer): Map<string, string> {
    ...
    if (lines.shift() !== "GET /qualification HTTP/1.1") {
...
export function createFragmentedUtf8Evidence(): FragmentedUtf8Evidence {
    ...
    if (codePointOffset === -1) {
        throw new Error("WebSocket qualification message lost its split code point");
    }
    const fragments = [
 ...
 * @param scenario Scenario selected by a focused qualification test.
    ...
    switch (scenario) {
        ...
        case "oversized-text": {
            frames = [
                encodeServerFrame(
                    0x01,
                    Buffer.alloc(oversizedQualificationMessageBytes, 0x61)
                ),
            ];
            break;
        }
