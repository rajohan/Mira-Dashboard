...
} from "./nativeWebSocketQualification.ts";
...
describe("Bun native WebSocket RFC 6455 qualification", () => {
    ...
    test("bounds an otherwise valid assembled native text message and closes with 1009", async () => {
        ...
        expect(evidence.error).toMatchObject({
            actualBytes: oversizedQualificationMessageBytes,
            eventCounts: {
                closes: 0,
                errors: 0,
                messages: 1,
    ...
    test("fails qualification when scoped native close does not cooperate", async () => {
        const fixture = createNonCooperatingCloseFixture();
        const exit = await Effect.runPromiseExit(
            observeNativeWebSocket("ws://127.0.0.1/qualification", fixture.factory)
        );
