/** Public readiness state for one qualification release. */
export interface QualificationReadinessSnapshot {
    releaseId: string;
    status: "not-ready" | "ready";
}

/** Explicit readiness lifecycle used by rolling-release qualification servers. */
export class QualificationReleaseReadiness {
    readonly #releaseId: string;
    #phase: "ready" | "starting" | "stopping" = "starting";

    constructor(releaseId: string) {
        if (releaseId.length === 0) {
            throw new Error("Qualification release ID must not be empty");
        }
    ...
    markReady(expectedReleaseId: string): void {
        if (expectedReleaseId !== this.#releaseId) {
            throw new Error("Cannot mark an unexpected qualification release ready");
        }
        if (this.#phase === "stopping") {
            throw new Error("Cannot mark a stopping qualification release ready");
        }
        this.#phase = "ready";
    }
     ...
     */
    snapshot(): QualificationReadinessSnapshot {
        return {
