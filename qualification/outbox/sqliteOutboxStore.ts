...
const outboxSchemaStatements = [
    ...
    `CREATE TABLE qualification_outbox_events (
        claim_owner TEXT,
        ...
        record_id INTEGER NOT NULL UNIQUE
            REFERENCES qualification_records(id) ON DELETE CASCADE,
        state TEXT NOT NULL DEFAULT 'pending',
        CONSTRAINT qualification_outbox_state_check CHECK (
    ...
    `CREATE INDEX qualification_outbox_claim_idx
        ON qualification_outbox_events (state, lease_until, id)`,
    `CREATE TABLE qualification_outbox_deliveries (
        delivered_at INTEGER NOT NULL,
        event_id INTEGER NOT NULL UNIQUE
            REFERENCES qualification_outbox_events(id) ON DELETE RESTRICT,
        id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
...
const qualificationCountStatements = Object.freeze({
    qualification_outbox_deliveries:
        "SELECT count(*) AS count FROM qualification_outbox_deliveries",
    qualification_outbox_events:
        "SELECT count(*) AS count FROM qualification_outbox_events",
    qualification_records: "SELECT count(*) AS count FROM qualification_records",
});

export type QualificationTableName = keyof typeof qualificationCountStatements;

export class QualificationSqliteContentionError extends Data.TaggedError(
...
export class QualificationSqliteUnavailableError extends Data.TaggedError(
    "QualificationSqliteUnavailableError"
)<{
    readonly cause: unknown;
    readonly code: string;
}> {}

export type QualificationSqliteOperationError =
    | QualificationSqliteContentionError
    | QualificationSqliteUnavailableError;
...
export interface QualificationOutboxSnapshot {
...
/**
 * Converts Bun SQLite failures into stable qualification failure tags.
 * @param error Unknown thrown value.
 * @returns A classified SQLite failure, or undefined for non-SQLite defects.
 */
export function classifyQualificationSqliteError(
    error: unknown
): QualificationSqliteOperationError | undefined {
    const code = sqliteErrorCode(error);
    if (code === undefined) return undefined;
    return isContentionCode(code)
        ? new QualificationSqliteContentionError({ cause: error, code })
        : new QualificationSqliteUnavailableError({ cause: error, code });
}

const isContentionError = Predicate.isTagged("QualificationSqliteContentionError");
 ...
 */
export function retryQualificationSqliteOperation<A>(
    operation: () => A
): Effect.Effect<A, QualificationSqliteOperationError> {
    const attempt = Effect.suspend(() => {
        try {
            return Effect.succeed(operation());
        } catch (error) {
            const failure = classifyQualificationSqliteError(error);
...
/**
 * Opens one strict file-backed qualification connection.
 ...
 */
export function openQualificationOutboxDatabase(
    databasePath: string,
 ...
 * @param database Writable qualification database.
 */
export function initializeQualificationOutboxDatabase(database: Database): void {
    const journalMode = database
        .query<JournalModeRow, []>("PRAGMA journal_mode = WAL")
        .get()?.journal_mode;
    if (journalMode?.toLowerCase() !== "wal") {
        throw new Error("Qualification database did not enter WAL mode");
    }
 ...
 * @param database Open qualification database.
 * @returns Lowercase SQLite journal mode.
 */
export function readQualificationJournalMode(database: Database): string {
 ...
 */
export function appendQualificationOutboxBatch(
    database: Database,
    producerId: string,
    count: number,
    createdAt: number
    ...
    const insertRecord = database.prepare<never, [string, number, string]>(
        `INSERT INTO qualification_records (producer_id, producer_sequence, payload)
         VALUES (?, ?, ?)`
    );
    const insertEvent = database.prepare<never, [number, number]>(
        `INSERT INTO qualification_outbox_events (record_id, created_at)
         VALUES (?, ?)`
    );
    try {
 ...
 */
export function claimQualificationOutboxBatch(
    database: Database,
    workerId: string,
    now: number,
    leaseUntil: number,
    ...
    const select = database.prepare<IdRow, [number, number]>(
        `SELECT id
         FROM qualification_outbox_events
         WHERE state = 'pending'
    ...
    const update = database.prepare<never, [string, number, number, number]>(
        `UPDATE qualification_outbox_events
 ...
 */
export function deliverQualificationOutboxClaims(
    database: Database,
    workerId: string,
    deliveredAt: number
): readonly number[] {
    ...
    const insertDelivery = database.prepare<never, [number, string, number]>(
        `INSERT INTO qualification_outbox_deliveries (event_id, worker_id, delivered_at)
         VALUES (?, ?, ?)`
    );
 ...
 */
export function readQualificationOutboxSnapshot(
    database: Database
): QualificationOutboxSnapshot {
    const eventIds = database
        .query<IdRow, []>("SELECT id FROM qualification_outbox_events ORDER BY id")
        .all()
        .map((row) => row.id);
    const deliveredEventIds = database
        .query<IdRow, []>(
            "SELECT event_id AS id FROM qualification_outbox_deliveries ORDER BY event_id"
        )
        .all()
        .map((row) => row.id);
 ...
 */
export function readQualificationDeliveryLatencies(
    database: Database
): readonly number[] {
    return Object.freeze(
        database
            .query<DeliveryLatencyRow, []>(
                `SELECT events.delivered_at - events.created_at AS latencyMs
                 FROM qualification_outbox_events AS events
 ...
 */
export function readQualificationIntegrityCheck(database: Database): string {
    const row = database
 ...
 */
export function createQualificationOutboxBackup(
    database: Database,
    backupPath: string
): void {
 ...
 * @param database Open qualification database.
 * @param tableName Allowlisted qualification table.
 * @returns Table row count.
 */
export function countQualificationRows(
    database: Database,
    tableName: QualificationTableName
): number {
    const row = database
        .query<CountRow, []>(qualificationCountStatements[tableName])
        .get();
