...
import { TerminalOutput } from "../components/features/terminal/TerminalOutput";
...
/** Focuses the command input after React applies the current update. */
function scheduleInputFocus(inputRef: RefObject<HTMLInputElement | undefined>): void {
    queueMicrotask(() => inputRef.current?.focus());
}

/**
 * Renders the terminal UI.
 * @returns Rendered the terminal UI.
 */
export function Terminal() {
    ...
    const outputRef = useRef<HTMLDivElement | undefined>(undefined);
    ...
    const submittedJobEntry = history.find((entry) => entry.jobId === submittedJobId);
    const currentJobId =
        submittedJobEntry?.status === "done" ? undefined : submittedJobId;
        ...
        if (trimmedCommand === "cd" || trimmedCommand.startsWith("cd ")) {
            ...
            try {
                ...
                addCommand({
                    ...
                    code: 1,
                    stdout: "",
                    stderr: messageFromError(error, "Failed to change directory"),
                    startedAt: Date.now(),
                    endedAt: Date.now(),
                });
            }
