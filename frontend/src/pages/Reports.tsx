...
function ReportsContent({ linkedReportId }: { linkedReportId: number | undefined }) {
    ...
    const reportsQuery = useReports(filter === "all" ? {} : { type: filter });
    const reports = [...(reportsQuery.data?.items ?? [])].toSorted(
        (a, b) => reportTimestamp(b) - reportTimestamp(a)
    );
    const selectedReportQuery = useReport(selectedId ?? reports[0]?.id);
    ...
    const selectedReport =
        ...
        (isAwaitingLinkedReport ? undefined : reportItems[0]);
    const effectiveSelectedId = selectedReport?.id ?? selectedId;
    let content: ReactNode;
    if (reportsQuery.isLoading) {
        content = <LoadingState message="Loading reports..." />;
...
/**
 * Renders the reports UI and resets selection when a new deep link is opened.
 * @returns Rendered the reports UI and resets selection when a new deep link is opened.
 */
export function Reports() {
    ...
    return (
        <ReportsContent
            key={linkedReportId === undefined ? "reports" : linkedReportId}
