 ...
 * @returns Rendered the layout UI.
 */
export function Layout({ children }: LayoutProperties) {
    const location = useLocation();
    const pageScrollRef = useRef<HTMLDivElement>(null);
    const sidebarId = useId();
    const [sidebarOpenPath, setSidebarOpenPath] = useState<string | undefined>();
        ...
        >
            <div className="flex items-center justify-between gap-3 border-b border-primary-700 p-4">
                ...
                <button
                    ...
                    onClick={() => setSidebarOpenPath(undefined)}
                >
                    <X size={20} />
                </button>
            </div>
        ...
        <div className="flex h-screen overflow-hidden bg-primary-900 text-primary-50">
            ...
            <main className="flex min-w-0 flex-1 flex-col overflow-hidden bg-primary-900">
                <AppHeader
                    ...
                    sidebarId={sidebarId}
                    onToggleSidebar={() =>
                        setSidebarOpenPath((openPath) =>
                            openPath === location.pathname ? undefined : location.pathname
                        )
                    }
                />
                <div ref={pageScrollRef} className="min-h-0 flex-1 overflow-auto">
                    {children}
                </div>
            </main>
        </div>
