 ...
 * @returns Rendered the files sidebar UI.
 */
export function FilesSidebar({
    ...
    let fileTreeContent: ReactNode;
    if (rootLoading && files.length === 0) {
        fileTreeContent = <div className="p-2 text-sm text-primary-400">Loading...</div>;
    } else if (files.length === 0) {
        fileTreeContent = (
            <div className="p-2 text-sm text-primary-400">No files found</div>
        );
    } else {
        fileTreeContent = files
            .toSorted((a, b) => {
