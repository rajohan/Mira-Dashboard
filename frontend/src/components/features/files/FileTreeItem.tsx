...
import type { FileNode } from "../../../hooks/useFileExplorerState";
 ...
 * @returns Rendered the file tree item UI.
 */
export function FileTreeItem({
    ...
    const sortedChildren =
        isExpanded && node.type === "directory" && node.children
            ...
            : [];
    let directoryIndicator: ReactNode = <span className="w-3.5 shrink-0" />;
    if (hasChildren) {
        directoryIndicator = isExpanded ? (
        ...
        directoryIndicator = (
            <RefreshCw size={14} className="shrink-0 animate-spin text-primary-400" />
        );
    }
