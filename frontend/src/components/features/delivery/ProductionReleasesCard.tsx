 ...
 * @returns Rendered one managed release slot.
 */
            ...
            {release ? (
                <>
                    <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="mt-2 line-clamp-2 text-sm font-medium wrap-break-word text-primary-200 hover:text-primary-50"
                    >
                        {release.commitTitle}
                    </a>
 ...
 * @returns Rendered managed release state and the manual rollback control.
 */
export function ProductionReleasesCard({
    ...
    if (error) {
        rollbackReason = messageFromError(error, "Release status is unavailable");
    } else if (isActionPending) {
        rollbackReason = "Another Dashboard action is in progress";
    } else if (release?.rollback.reason) {
        rollbackReason = release.rollback.reason;
    } else if (!release) {
        rollbackReason = "Managed release status is still loading";
    }
    const rollbackReasonId = rollbackReason ? "rollback-disabled-reason" : undefined;
    let releaseBadgeVariant: ComponentProps<typeof Badge>["variant"] = "warning";
    let releaseBadgeLabel = "Checking releases";
    if (error) {
        releaseBadgeVariant = "error";
        releaseBadgeLabel = "Status unavailable";
    } else if (release?.current) {
        releaseBadgeVariant = "success";
        releaseBadgeLabel = "Managed release active";
    }
    ...
    const previousSlotLabel =
        release?.previous && !release.rollback.available
            ? "Previous slot"
            : "Rollback target";
    let previousBadgeVariant: ComponentProps<typeof Badge>["variant"] = "default";
    let previousBadgeLabel = "Unavailable";
    if (release?.previous) {
        previousBadgeVariant = release.rollback.available ? "warning" : "error";
        previousBadgeLabel = release.rollback.available ? "Previous" : "Not eligible";
    }

    let checkoutBadgeVariant: ComponentProps<typeof Badge>["variant"] = "default";
    let checkoutBadgeLabel = "Checking";
    if (checkoutCommit) {
        checkoutBadgeVariant = isMainCheckoutActive ? "success" : "info";
        checkoutBadgeLabel = isMainCheckoutActive ? "Active" : "Deploy available";
    }
        ...
        <Card variant="bordered" className="space-y-3">
            ...
            {error ? (
                <p className="text-sm text-red-300">
                    {messageFromError(error, "Release status is unavailable")}
                </p>
            ) : undefined}
            ...
            <div className="grid gap-2 lg:grid-cols-3">
                ...
                <ReleaseSlot
                    ...
                    badge={
                        <Badge variant={previousBadgeVariant}>{previousBadgeLabel}</Badge>
                    }
                />
                <div className="rounded border border-primary-700 bg-primary-900/40 p-3">
                    <div className="flex items-start justify-between gap-2">
