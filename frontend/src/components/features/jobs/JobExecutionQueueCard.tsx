 ...
 * @returns Job execution queue card result.
 */
export function JobExecutionQueueCard({
    ...
    const oldestWait = Number.isFinite(oldestQueuedTimestamp)
        ...
        : "None";
    let workerBadgeVariant: ComponentProps<typeof Badge>["variant"] = "error";
    let workerBadgeLabel = "Worker offline";
    if (queue.isLoading) {
        ...
        <Card variant="bordered" className={cn("space-y-3 p-3 sm:p-4", className)}>
            ...
            {queue.error ? (
                <Alert variant="warning">
                    ...
                    {messageFromError(queue.error, "Unknown queue error")}
                </Alert>
            ) : undefined}
