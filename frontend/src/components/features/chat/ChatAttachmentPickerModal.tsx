 ...
 * @returns Rendered an icon or thumbnail for an attachment waiting to be sent.
 */
 ...
 * @returns Rendered the custom file selection and drop experience for chat attachments.
 */
export function ChatAttachmentPickerModal({
    ...
    useEffect(() => {
        ...
        const preventFileDropOutsideZone = (event: globalThis.DragEvent) => {
            if (
                ...
                (event.target instanceof Node &&
                    dropZoneRef.current?.contains(event.target))
            ) {
                return;
            }
        ...
        const unsubscribeDragEnter = subscribeToGlobalEvent<globalThis.DragEvent>(
            "dragenter",
            preventFileDropOutsideZone,
            listenerOptions
        );
