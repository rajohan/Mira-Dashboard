 ...
 * @returns Formatted tool arguments for display.
 */
function formatToolArguments(toolCall: ChatToolCallDisplay): string {
    if (toolCall.arguments === undefined) {
        return "";
    }
    return serializeForDisplay(toolCall.arguments, {
        fallback: "[Tool arguments could not be serialized]",
        indentation: 2,
    });
}
 ...
 * @returns Formatted a compact tool display name.
 */
...
function toolCallSummary(toolCall: ChatToolCallDisplay): string | undefined {
    ...
    let command = typeof record.cmd === "string" ? record.cmd : undefined;
        ...
        >
            {blocks?.map((block) => (
                <pre
                    key={
                        block.id ||
                        `${block.snapshot ? "snapshot" : "delta"}:${block.text}`
                    }
 ...
 * @returns Rendered the detail block UI.
 */
 ...
 * @returns Rendered a collapsible tool bubble with its status always visible.
 */
 ...
 * @returns Rendered a labeled nested tool section.
 */
 ...
 * @returns Rendered the chat message details UI.
 */
        ...
        <div className="mt-1.5 space-y-1.5">
            ...
            {shouldShowToolCalls
                ? message.toolCalls?.map((toolCall) => {
                      ...
                      const toolKey = `${toolKeyPrefix}:call:${
                          toolCall.id ||
                          `${toolCall.name}:${formattedArguments}:${summary || ""}`
                      }`;
                      let status = toolResult ? "Completed" : "Running";
