 ...
 * @returns Rendered the reports overview card UI.
 */
export function ReportsOverviewCard() {
    ...
    if (isLoading) {
        content = <div className="text-sm text-primary-300">Loading reports…</div>;
    } else if (isError && !data) {
        content = <div className="text-sm text-rose-300">Reports unavailable.</div>;
    } else {
        content = (
            <div className="space-y-2 text-sm text-primary-200">
                ...
                <div className="flex items-center justify-between">
                    <span>Errors</span>
                    <span className={errorCount > 0 ? "text-red-300" : "text-green-300"}>
                        {errorCount}
                    </span>
                </div>
                ...
                <div className="flex items-center justify-between">
                    ...
                    <Badge
                        variant={
                            latestReport ? statusVariant(latestReport.status) : "default"
                        }
                    >
