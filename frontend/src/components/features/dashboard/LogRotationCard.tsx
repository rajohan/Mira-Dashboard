 ...
 * @returns Rendered the log rotation card UI.
 */
export function LogRotationCard() {
    ...
    const requestError =
        !failedRealRun && realRun.error
            ? { error: messageFromError(realRun.error, "Log rotation failed") }
            : undefined;
    ...
    let lastRunResult = "No recorded run yet";
    if (lastRun) {
        lastRunResult = `${lastRun.rotatedFiles} rotated · ${lastRun.errors.length} errors`;
    } else if (status.isLoading) {
        lastRunResult = "Loading...";
    }

    let realRunButtonLabel = "Run real now";
    if (realRun.isPending) {
        realRunButtonLabel = "Queueing...";
    } else if (logRotationJob?.isQueued) {
        realRunButtonLabel = "Queued";
    } else if (logRotationJob?.isRunning) {
        realRunButtonLabel = "Running...";
    }
