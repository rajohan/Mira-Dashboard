 ...
 * @returns Rendered the Docker overview card UI.
 */
export function DockerOverviewCard() {
    ...
    if (isLoading) {
        content = <div className="text-sm text-primary-300">Loading Docker cache…</div>;
    } else if (docker === undefined) {
        content = <div className="text-sm text-rose-300">Docker cache unavailable.</div>;
    } else {
        content = (
            <div className="space-y-2 text-sm text-primary-200">
                ...
                <div className="flex items-center justify-between">
                    <span>Unhealthy</span>
                    <span className={unhealthy > 0 ? "text-red-300" : "text-green-300"}>
                        {unhealthy}
                    </span>
                </div>
                <div className="flex items-center justify-between">
