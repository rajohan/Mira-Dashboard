 ...
 * @returns Rendered the Git overview card UI.
 */
export function GitOverviewCard() {
    ...
    const offMainRepos = repos.filter(
        (repo) => repo.exists && repo.branch && repo.branch !== DEFAULT_BRANCH
    );
    let content: ReactNode;
    if (isLoading) {
        content = <div className="text-sm text-primary-300">Loading git cache…</div>;
    } else if (git === undefined) {
        content = <div className="text-sm text-rose-300">Git cache unavailable.</div>;
    } else {
        content = (
...
function GitRepoRow({ repo }: { repo: GitRepoSummary }) {
    ...
    let statusVariant: ComponentProps<typeof Badge>["variant"] = "success";
    ...
    let repositorySummary = repo.branch || "unknown branch";
    if (isMissing) {
        repositorySummary = "repository unavailable";
    } else if (repo.statusSummary.total > 0) {
        const changeSuffix = repo.statusSummary.total === 1 ? "" : "s";
        repositorySummary += ` · ${repo.statusSummary.total} change${changeSuffix}`;
    } else {
        repositorySummary += " · no changes";
    }
