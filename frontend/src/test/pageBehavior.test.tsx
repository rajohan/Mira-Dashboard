...
import { defaultDisableUntilDraft } from "../pages/jobPageUtilities";
...
function apiResponse(url: string, method: string, init?: RequestInit) {
    ...
    if (method === "POST" && url.startsWith("/api/cache/") && url.endsWith("/refresh")) {
        ...
        return Response.json({ entry: cacheEnvelopeFixture(key, {}), isOk: true });
    }
    ...
    if (method === "POST" && url === "/api/terminal/cd") {
        const body = parseRequestBody(init);
        if (body.path === "/tmp") {
            return Response.json({ newCwd: "/tmp" });
        }
        return Response.json(
            {
                error: {
    ...
    if (url === "/api/agents/status") {
        return Response.json({
            agents: [
                ...
                { id: "ops", status: "idle", model: "codex" },
                {
                    id: "researcher",
                    status: "offline",
                    model: "codex",
    ...
    if (url === "/api/cache/heartbeat") {
        return Response.json({
            ...
            dashboardJobs: [],
            entries: [
                cacheEnvelopeFixture("weather.spydeberg", {}, { source: "weather" }),
                cacheEnvelopeFixture("quotas.summary", {}, { source: "quota" }),
                cacheEnvelopeFixture("moltbook.home", {}, { source: "moltbook" }),
                cacheEnvelopeFixture("git.workspace", {}, { source: "git" }),
                cacheEnvelopeFixture("system.host", {}, { source: "system" }),
    ...
    if (url === "/api/cache/status") {
        ...
        return Response.json({
            ...
            entries: keys.map(([key, source]) =>
                cacheEnvelopeFixture(key, null, { source })
            ),
            generatedAt: "2026-06-24T08:00:00.000Z",
        });
    }
    ...
    if (url === "/api/config") {
        if (method === "PUT") {
            ...
            return Response.json({ isOk: true, result: {} });
        }
        return Response.json({
    ...
    if (url === "/api/cache/system.host") {
        return Response.json(
            cacheEnvelopeFixture(
                "system.host",
                { version: { current: "2026.6.9", latest: "2026.6.9" } },
                { source: "system" }
            )
        );
    }
...
describe("Mira Dashboard pages", () => {
    ...
    it("renders the main data pages from their API contracts", async () => {
        ...
        > = [
            ...
            [createElement(Logs), "1 entry", { withSocket: true }],
    ...
    it("drives logs page loading, searching, level filtering, and clearing", async () => {
        ...
        try {
            ...
            await user.click(screen.getByRole("button", { name: "OpenClaw logs" }));
            await waitFor(() => {
    ...
    it("drives docker page container, updater, prune, delete, and console flows", async () => {
        ...
        await waitFor(() => {
            expect(
                screen.queryByRole("dialog", { name: /dashboard logs/i })
            ).not.toBeInTheDocument();
        });
