...
describe("chat input media", () => {
    ...
    it("does not restore sent input after voice-file transcription starts", async () => {
        ...
        try {
            ...
            await act(async () => {
                transcriptionResponse.resolve(
                    Response.json({ provider: "elevenlabs", text: "new transcript" })
                );
                await transcription;
            });
    ...
    it("transcribes voice files and reports provider and input failures", async () => {
        ...
        const input = document.createElement("input");
        result.current.voiceFileInputRef.current = input;
        ...
        try {
            ...
            defineFetch(
                jest.fn(() =>
                    Promise.try(() => new Response("not json", { status: 500 }))
                ) as unknown as typeof fetch
            );
            await act(async () => {
                await result.current.handleVoiceFileSelected(
    ...
    it("ignores a transcription that completes after the session changes", async () => {
        ...
        result.current.fileInputRef.current = attachmentInput;
        result.current.voiceFileInputRef.current = voiceInput;
        ...
        try {
            ...
            await act(async () => {
                response.resolve(
                    Response.json({ provider: "elevenlabs", text: "stale transcript" })
                );
                await transcription;
            });
    ...
    it("falls back to the voice file picker when direct recording is unavailable", async () => {
        ...
        const click = jest.fn();
        result.current.voiceFileInputRef.current = {
            click,
    ...
    it("records, stops, transcribes and releases microphone tracks", async () => {
        ...
        try {
            defineFetch(
                jest.fn(() =>
                    Promise.try(() =>
                        Response.json({ provider: "elevenlabs", text: "recorded" })
                    )
                ) as unknown as typeof fetch
            );
            Object.defineProperty(globalThis, "MediaRecorder", {
            ...
            const getUserMedia = jest.fn(() => Promise.try(() => stream));
            Object.defineProperty(navigator, "mediaDevices", {
                configurable: true,
                value: { getUserMedia },
            });
