...
describe("shared component helpers", () => {
    ...
    it("renders markdown links, images, tables, fallbacks, and code blocks", () => {
        ...
        expect(screen.getByRole("link", { name: "Dashboard image" })).toHaveAttribute(
            "href",
    ...
    it("drives chat composer attachments, slash suggestions, emoji, and submit controls", async () => {
        ...
        expect(
            screen.queryByRole("button", { name: /close emoji picker/i })
        ).not.toBeInTheDocument();
        await user.click(screen.getByRole("button", { name: /insert emoji/i }));
    ...
    it("handles chat slash commands without rendering the page", async () => {
        ...
        const selectedSessionKeyRef = { current: "agent:main:main" };
    ...
    it("renders chat messages list helpers and primary row actions", async () => {
        ...
        fireEvent.pointerDown(messagesContainerRef.current!, { clientX: 95 });
        ...
        const fetchMock = jest.fn((input: RequestInfo | URL, init?: RequestInit) => {
            return Promise.try(() => {
                ...
                if (url === "/api/config-files/openclaw.json" && method === "PUT") {
                    savedContent = (
                        JSON.parse(requestBodyText(init?.body)) as { content: string }
                    ).content;
                    return Response.json({
                        isSuccess: true,
                        modified: "2026-07-28T12:01:00.000Z",
            ...
            return Promise.try(() => {
                ...
                if (method === "POST") {
                    ...
                    return url.endsWith("/clear-needs-attention")
                        ? Response.json({ cleared: job, isOk: true })
