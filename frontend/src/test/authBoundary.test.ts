...
describe("Dashboard authentication boundary", () => {
        ...
        Object.defineProperty(globalThis, "fetch", {
            ...
            value: jest.fn((input: RequestInfo | URL) => {
                return Promise.try(() => {
                    ...
                    throw new Error(
                        `Unexpected cross-tab auth request: ${requestUrl(input)}`
                    );
                });
            }),
            writable: true,
    ...
    it("does not recover a transport from a refresh invalidated by logout", () => {
        authActions.setSession({
            authenticated: true,
            isBootstrapRequired: false,
            user: { id: 1, username: "raymond" },
        ...
        try {
            ...
            expect(recovery).resolves.toBe(false);
            expect(authStore.state.isAuthenticated).toBe(false);
            expect(unauthorizedHandler).toHaveBeenCalledTimes(1);
        } finally {
            removeEventListener(UNAUTHORIZED_EVENT_NAME, unauthorizedHandler);
    ...
    it("does not replay after an unsigned same-user session replacement", () => {
        authActions.setSession({
            authenticated: true,
            isBootstrapRequired: false,
            session: {
        ...
        try {
            expect(recoverOrHandleUnauthorizedSession()).resolves.toBe(false);
    ...
    it("preserves a valid session after a transient recovery refresh failure", () => {
        authActions.setSession({
            authenticated: true,
            isBootstrapRequired: false,
            session: {
    ...
    it("does not replay an old user's transport after an authenticated identity change", () => {
        authActions.setSession({
            authenticated: true,
            isBootstrapRequired: false,
            session: {
