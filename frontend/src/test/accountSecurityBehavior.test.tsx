...
function installAccountFetch(
    ...
    const fetchMock = jest.fn(
        async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
            ...
            const body =
                typeof init?.body === "string" ? parseJsonText(init.body) : undefined;
...
describe("Dashboard account security", () => {
        ...
        const { calls } = installAccountFetch(
            ...
            (url, method, body) => {
                if (
                    ...
                    return Response.json({ isOk: true, verifiedAt: VERIFIED_AT });
                }
    ...
    it("validates password confirmation before changing the password", async () => {
        const { calls } = installAccountFetch(
            () =>
                summary({
                    enabled: false,
        ...
        expect(
            calls.some(
                (call) =>
                    call.url === "/api/account/security/password/change" &&
                    call.method === "POST"
            )
        ).toBe(true);
        act(() => {
