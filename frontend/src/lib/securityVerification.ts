...
export const SECURITY_VERIFICATION_DEADLINE_REFRESH_EVENT_NAME =
    "mira:security-verification-deadline-refresh";
...
/** Gives the active dialog and every held request a fresh flow deadline. */
export function refreshSecurityVerificationDeadline(): void {
    dispatchEvent(new Event(SECURITY_VERIFICATION_DEADLINE_REFRESH_EVENT_NAME));
}
 ...
 * @returns Wait for security verification outcome result.
 */
export function waitForSecurityVerificationOutcome(
    ...
    return new Promise((resolve) => {
        ...
        function settle(outcome: SecurityVerificationOutcome): void {
            if (isSettled) return;
            isSettled = true;
            ...
            removeEventListener(
                SECURITY_VERIFICATION_DEADLINE_REFRESH_EVENT_NAME,
                startTimeout
            );
            resolve(outcome);
        }
        function startTimeout(): void {
            ...
            timeoutRef.current = setTimeout(() => settle("cancelled"), timeoutMs);
        }
        ...
        addEventListener(SECURITY_VERIFICATION_DEADLINE_REFRESH_EVENT_NAME, startTimeout);
