...
export function getCronJobName(job: CronJob): string {
    return job.name || getCronJobId(job) || "Unnamed job";
}

/**
 * Sorts cron jobs.
 * @param jobs Jobs value.
 * @returns Sorted cron jobs.
 */
 ...
 * @returns Formatted cron timestamp for display.
 */
 ...
 * @param value Value to process.
 * @returns Formatted cron last status for display.
 */
