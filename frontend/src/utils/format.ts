 ...
 * @returns Formatted a date/time value with app timezone date and time fields.
 */
export function formatDate(date: Date | string | number): string {
    try {
        const d = toDateValue(date);
        if (Number.isNaN(d.getTime())) return String(date);
        const parts = appTimeZoneParts(d);
 ...
 * @param date Date value.
 * @returns Formatted a date/time value as an Oslo clock time.
 */
 ...
 * @returns Formatted a date/time value as a compact app-timezone date stamp.
 */
 ...
 * @returns Formatted the provided Date with app-timezone time fields.
 */
 ...
 * @returns Formatted the provided Date with app-timezone date fields.
 */
export function formatOsloDate(date: Date): string {
    try {
        if (Number.isNaN(date.getTime())) return "Unknown date";
        const parts = appTimeZoneParts(date);
        return `${parts.weekday} ${pad2(parts.day)}. ${appTimeZoneShortMonth(date)} ${
            parts.year
        }`;
    } catch {
        return "Unknown date";
 ...
 * @returns Formatted a date/time value as a short app-timezone weekday label.
 */
export function formatWeekdayShort(date: Date): string {
    try {
        if (Number.isNaN(date.getTime())) return "---";
        return appTimeZoneShortWeekday(date);
    } catch {
        return "---";
    }
}
 ...
 * @returns Converted app-timezone calendar fields to a timestamp when that local time exists.
 */
...
function referenceUtcDateParts(referenceDate?: Date | string | number) {
    const reference = new Date(referenceDate ?? Date.now());
    const validRef = Number.isNaN(reference.getTime()) ? new Date() : reference;
    return {
        day: validRef.getUTCDate(),
        month: validRef.getUTCMonth() + 1,
        year: validRef.getUTCFullYear(),
    };
}
 ...
 * @returns Converted a UTC HH:mm daily schedule value to app-timezone HH:mm.
 */
export function formatUtcTimeOfDayInAppTimeZone(
    ...
    const parts = referenceUtcDateParts(referenceDate);
 ...
 * @param timeOfDay Time of day value.
 * @param referenceDate Reference date value.
 * @returns Converted an app-timezone HH:mm daily schedule value to UTC HH:mm.
 */
 ...
 * @returns Formatted milliseconds as a compact duration string.
 */
 ...
 * @returns Formatted uptime seconds as days, hours, or minutes.
 */
 ...
 * @returns Formatted bytes as a human-readable binary size.
 */
 ...
 * @returns Formatted a numeric load value to two decimal places.
 */
 ...
 * @param max Max value.
 * @returns Formatted token counts using compact notation.
 */
 ...
 * @returns Formatted nullable token counts with an em dash fallback.
 */
 ...
 * @param current Current value.
 * @param max Max value.
 * @returns Calculated token usage percentage from used and limit values.
 */
