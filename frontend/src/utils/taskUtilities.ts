 ...
 * @returns a stable task column for movement controls.
 */
export function normalizeTaskDetailColumn(column?: ColumnId): ColumnId {
    return column ?? "todo";
}

/**
 * Formats a task status badge without coercing a missing column.
 * @returns Formatted a task status badge without coercing a missing column.
 */
export function formatTaskColumnBadge(column?: ColumnId): string {
 ...
 * @param taskNumber Task number value.
 * @returns searchable task number variants.
 */
