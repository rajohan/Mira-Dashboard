...
/**
 * Fetches session.
 * @returns Promise resolving to the fetch session result.
 */
 ...
 */
export function useAuthUser(): DashboardUser | undefined {
    return useSelector(authStore, (state) => state.user);
}

/**
 * Provides the opaque identity of the active browser session.
 * @returns The opaque identity of the active browser session.
 */
