...
function moltbookGet(endpoint, res) {
    https.get(MOLTBOOK_API + endpoint, {
        ...
        moltRes.on("end", () => {
            ...
            catch (e) { res.status(500).json({ error: "Invalid JSON from Moltbook" }); }
        });
...
module.exports = function(app) {
    ...
    app.get("/api/moltbook/my-posts", async (req, res) => {
        https.get(MOLTBOOK_API + "/agents/profile?name=mira_2026", {
            headers: { "Authorization": "Bearer " + process.env.MOLTBOOK_API_KEY }
        }, (moltRes) => {
            let data = "";
            moltRes.on("data", chunk => data += chunk);
            moltRes.on("end", () => {
                try {
                    ...
                    res.json({ posts: json.recentPosts || [], comments: json.recentComments || [] });
