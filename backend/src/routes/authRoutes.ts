...
function validateSecondFactorCode(code: unknown): string | undefined {
...
export const authRoutes = {
    ...
    "/api/auth/login": {
        POST: async (request: Request, server: Server<unknown>) => {
            ...
            const throttled = authenticationThrottled("login-password", username);
    ...
    "/api/auth/login/totp": {
        POST: async (request: Request, server: Server<unknown>) => {
            ...
            const body = await readAuthBody(request);
            if (body instanceof Response) return body;
            const code = validateSecondFactorCode(body.code);
            const factor = code
    ...
    "/api/auth/login/recovery": {
        POST: async (request: Request, server: Server<unknown>) => {
            ...
            const code = validateSecondFactorCode(body.code);
            const verified =
                code && (await verifyRecoveryCodeForUser(attempt.pending.userId, code));
            if (!verified) {
                recordAuthenticationFailure("second-factor", attempt.pending.userId);
                return failedSecondFactor(request, server, attempt.pending);
            }
