...
function authenticationResponse(value: unknown): AuthenticationResponseJSON | undefined {
    if (!value || typeof value !== "object" || Array.isArray(value)) {
        return undefined;
    }
...
function authenticationThrottled(
...
export const accountSecurityRoutes = {
    "/api/account/security": {
        GET: (request: Request, server: Server<unknown>) => {
            void server;
            const context = requestContext(request);
            return context instanceof Response ? context : json(securitySummary(context));
        },
    },
    ...
    "/api/account/security/reauth/password": {
        POST: async (request: Request, server: Server<unknown>) => {
            ...
            const throttled = authenticationThrottled(
                "account-password",
                context.session.id
            );
    ...
    "/api/account/security/step-up/webauthn/verify": {
        POST: async (request: Request, server: Server<unknown>) => {
            ...
            if (!factor) {
                recordAuthenticationFailure("second-factor", context.session.id);
                return json({ error: "Invalid security-key response" }, { status: 400 });
            }
            clearAuthenticationFailures("second-factor", context.session.id);
    ...
    "/api/account/security/totp/confirm": {
        POST: async (request: Request, server: Server<unknown>) => {
            ...
            return confirmation.enabledMfa
                ? upgradeAfterFirstFactor(request, server, context, "totp", responseBody)
                : json(responseBody);
        },
    },
    ...
    "/api/account/security/webauthn/register/verify": {
        POST: async (request: Request, server: Server<unknown>) => {
            ...
            securityEvent("account.security-key-added", registration.credential.id, {
    ...
    "/api/account/security/webauthn/:credentialId": {
        DELETE: (request: ParametersRequest<"credentialId">, server: Server<unknown>) => {
            void server;
            const context = requestContext(request);
            if (context instanceof Response) return context;
            if (!hasRecentMfaVerification(context.session)) {
            ...
            securityEvent("account.security-key-removed", credentialId);
            return json({ isOk: true });
        },
    },
    ...
    "/api/account/security/recovery-codes/rotate": {
        POST: async (request: Request, server: Server<unknown>) => {
            ...
            securityEvent("account.recovery-codes-rotated", String(context.session.id), {
    ...
    "/api/account/security/sessions/:sessionId": {
        DELETE: (request: ParametersRequest<"sessionId">, server: Server<unknown>) => {
            const context = requestContext(request);
            if (context instanceof Response) return context;
            if (!canManageFactors(context.session)) {
                return recentVerificationRequired();
