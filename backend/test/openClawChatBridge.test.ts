...
describe("OpenClaw chat bridge", () => {
    ...
    it("cancels deferred completion when a filtered item resumes the parent", () => {
        ...
        try {
            ...
            const snapshot = bridge.snapshot(MAIN);
            expect(snapshot.completed).toBe(false);
            expect(snapshot.events).toHaveLength(eventCountBeforeContinuation);
            expect(deferredEnvelopes).toEqual([]);
        } finally {
            jest.useRealTimers();
        }
    ...
    it("releases failed send boundaries without clearing concurrent requests", () => {
        ...
        bridge.recordEvent(
            "agent",
            {
                data: { delta: "work before failed send" },
                runId: provisionalRunId,
                sessionKey: MAIN,
                stream: "thinking",
            },
    ...
    it("clears a runless live-steer boundary before reconnecting the active run", () => {
        ...
        expect(steerEcho.payload).toMatchObject({ runId: provisionalRunId });
