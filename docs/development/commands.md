...
| ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| `bun run bootstrap [production]`                                             | Complete first production installation from clean `main` and its CI-qualified immutable artifact      | Installs root policy/units and activates the first release          |
| `bun run bootstrap development [--doppler] [--no-start] [--with-browser]`    | Frozen development install, generated checks, isolated state preparation, then local start by default | Installs dependencies/state; optionally Chromium/Doppler start      |
...
passes explicit partitions rather than calling implementation files. Production bootstrap obtains
