...
The paths below are relative to the repository root. The retired source tree is not merged or
available through compatibility imports. Use one private Bun package and
...
The Overview exposes exactly nine fixed Service Actions through
`serviceActions.getStatus` and `serviceActions.request`: Dashboard web restart, Dashboard web and
worker restart, Dashboard worker restart, OpenClaw session cleanup, OpenClaw Gateway restart,
OpenClaw installation update, bounded host cleanup, host restart, and host update. The browser submits only
...
| `/`           | Compact host metrics and weather lead the operational overview. Quota, Git, unfinished tasks, jobs, reports, active incidents, three independent backup cards, Docker, database, logs, fixed Service Actions, application observability, and the generic saved-cache browser retain independently available data through transient refresh errors. |
