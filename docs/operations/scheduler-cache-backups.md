...
| reports and their linked notifications      | 365 days and at most 5,000 reports, excluding reports pinned by unread heartbeat incidents |
...
the retention bound is a crash-recovery fallback. Unread standalone
notifications are preserved. An unread heartbeat incident notification and its
linked report are preserved until the notification is read manually or by
incident recovery. Other report-linked notifications follow report retention.
The job does not run automatic `VACUUM`.
