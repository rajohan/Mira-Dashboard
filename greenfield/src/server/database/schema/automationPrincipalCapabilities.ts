...
export const automationPrincipalCapabilities = sqliteTable(
    ...
    (table) => [
        check(
            ...
            sql`${table.capability} IN ('agents:read', 'agents:write', 'jobs:read', 'jobs:write', 'monitoring:write', 'notifications:read', 'notifications:write', 'reports:read', 'reports:write', 'tasks:read', 'tasks:write')`
        ),
        check(
