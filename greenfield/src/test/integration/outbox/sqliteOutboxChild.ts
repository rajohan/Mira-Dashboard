...
import {
    ...
    claimIntegrationOutboxBatch,
    deliverIntegrationOutboxClaims,
    openIntegrationOutboxDatabase,
    retryIntegrationSqliteOperation,
...
class IntegrationChildArgumentError extends Data.TaggedError(
    "IntegrationChildArgumentError"
)<{
    readonly message: string;
}> {}

class IntegrationChildStatusWriteError extends Data.TaggedError(
    "IntegrationChildStatusWriteError"
)<{
    readonly cause: unknown;
}> {}

class IntegrationOutboxPollingExhaustedError extends Data.TaggedError(
    "IntegrationOutboxPollingExhaustedError"
)<{
    ...
    switch (kind) {
        ...
        case "claim-and-hold": {
            ...
            if (command.leaseUntil <= command.now) {
                throw new IntegrationChildArgumentError({
        ...
        default: {
            throw new IntegrationChildArgumentError({
                message: "Unrecognized SQLite outbox integration child command",
            });
        }
    }
}
...
): Effect.Effect<void, IntegrationChildStatusWriteError> {
    return Effect.tryPromise({
        catch: (cause) => new IntegrationChildStatusWriteError({ cause }),
        try: () => Bun.write(statusPath, `${JSON.stringify(status)}\n`),
    }).pipe(Effect.asVoid);
}

function runDrainCommand(
    database: ReturnType<typeof openIntegrationOutboxDatabase>,
    ...
    return Effect.gen(function* () {
        ...
        for (let poll = 0; poll < maximumPolls; poll += 1) {
            const claimedEventIds = yield* retryIntegrationSqliteOperation(() =>
                claimIntegrationOutboxBatch(
                    database,
                    command.workerId,
                    command.now,
            ...
            const deliveredEventIds = yield* retryIntegrationSqliteOperation(() =>
                deliverIntegrationOutboxClaims(database, command.workerId, command.now)
            );
            deliveredCount += deliveredEventIds.length;
            yield* Effect.sleep("1 millis");
        }
        return yield* Effect.fail(
            new IntegrationOutboxPollingExhaustedError({ maximumPolls })
        );
    });
}
    ...
    return Effect.scoped(
        Effect.gen(function* () {
            const database = yield* Effect.acquireRelease(
                Effect.sync(() => openIntegrationOutboxDatabase(command.databasePath)),
                (acquiredDatabase) => Effect.sync(() => acquiredDatabase.close(true))
            );

            switch (command.kind) {
                case "produce": {
                    const batch = yield* retryIntegrationSqliteOperation(() =>
                        appendIntegrationOutboxBatch(
                            database,
                            command.producerId,
                            command.count,
                ...
                case "claim-and-hold": {
                    const eventIds = yield* retryIntegrationSqliteOperation(() =>
                        claimIntegrationOutboxBatch(
                            database,
                            command.workerId,
                            command.now,
