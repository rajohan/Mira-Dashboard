...
const gatewayConnectClientSchema = v.strictObject({
    displayName: v.literal("Mira shutdown integration"),
    id: v.literal("gateway-client"),
    mode: v.literal("cli"),
    platform: v.literal("linux"),
    version: v.literal("integration"),
});
...
const gatewayConnectRequestSchema = v.strictObject({
    id: v.literal("shutdown-integration-connect"),
...
const gatewayHelloServerSchema = v.strictObject({
    connId: v.literal("shutdown-integration"),
...
function parseBoundedJson(text: string, maximumBytes: number): unknown {
    if (Buffer.byteLength(text, "utf8") > maximumBytes) {
        throw new Error("Shutdown integration Gateway frame exceeded its bound");
    }
...
export function createGatewayConnectRequest(nonce: string) {
    if (nonce.length === 0 || nonce.length > 128) {
        throw new Error("Shutdown integration Gateway nonce is invalid");
    }
    return {
        id: "shutdown-integration-connect" as const,
        method: "connect" as const,
        params: {
            auth: { token: "shutdown-fixture-token" as const },
            client: {
                displayName: "Mira shutdown integration" as const,
                id: "gateway-client" as const,
                mode: "cli" as const,
                platform: "linux" as const,
                version: "integration" as const,
            },
...
export function createGatewayHelloResponse() {
    return {
        ...
        payload: {
            ...
            protocol: 4 as const,
            server: {
                connId: "shutdown-integration" as const,
