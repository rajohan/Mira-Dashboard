...
export function openShutdownIntegrationDatabase(databasePath: string): Database {
    ...
    if (journalMode?.toLowerCase() !== "wal") {
        database.close(true);
        throw new Error("Shutdown integration database did not enter WAL mode");
    }
    ...
    return database
        .transaction(() => {
            ...
            if (activeLeaseCount !== 0) {
                throw new Error("A prior shutdown integration lease remains active");
            }
            ...
            if (recoveredGenerationCount === undefined) {
                throw new Error("Shutdown integration generation count is unavailable");
            }
            database
...
export function releaseShutdownWorkerLease(
    ...
    if (result.changes !== 1) {
        throw new Error("Shutdown integration worker lease was not released once");
    }
}
...
export function completeShutdownGeneration(
    ...
    if (
        ...
        throw new Error("Shutdown integration WAL checkpoint did not complete");
    }
}
