...
describe("SSE memory evidence CLI", () => {
    ...
    test("accepts only one absolute child result path", () => {
        expect(
            parseSseMemoryCliArguments([
                "--child",
                "--result=/tmp/integration-evidence/evidence.json",
                `--unit=${unitName}`,
            ])
        ).toEqual({
            mode: "child",
            resultPath: "/tmp/integration-evidence/evidence.json",
            unitName,
        });
    ...
    test("formats nested and aggregate CLI failures without losing causes", () => {
        const nested = formatSseMemoryEvidenceError(
        ...
        const aggregate = formatSseMemoryEvidenceError(
            new AggregateError(
