...
} from "../transport/realtime/eventFeed.ts";
...
const evidenceCookie = "mira_evidence=paused-native-client";
...
describe("paused native TLS SSE client", () => {
    ...
    test("propagates a paused TLS receive window to the bounded event queue", async () => {
        ...
        try {
            ...
            const payload = "x".repeat(integrationEventLimits.maximumPayloadBytes);
            for (
                let sequence = 1;
                sequence <= sseMemoryEvidencePolicy.scenario.maximumEventsPerRound;
                sequence += 1
            ) {
                eventFeed.publish({
