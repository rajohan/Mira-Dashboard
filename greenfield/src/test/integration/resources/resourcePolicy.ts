...
/** Fixed cgroup and load boundaries for the SSE memory evidence. */
export const sseMemoryEvidencePolicy = Object.freeze({
...
function assertExactLimit(label: string, actual: CgroupV2Limit, expected: number): void {
    if (actual !== expected) {
        throw new Error(
            `SSE memory evidence requires ${label}=${expected}; observed ${String(actual)}`
        );
    }
}
...
function assertAncestorLimit(
    ...
    if (actual !== "max" && actual < leafLimit) {
        throw new Error(
            `SSE memory evidence ancestor ${label} must not be stricter than ${leafLimit}; observed ${actual}`
        );
    }
}
...
export function assertCgroupAncestorResourcePolicy(
    ...
    if (ancestors.length === 0) {
        throw new Error("SSE memory evidence requires cgroup ancestor evidence");
    }
    ...
    for (const ancestor of ancestors) {
        ...
        if (
            ...
            throw new Error(
                `SSE memory evidence ancestor ${ancestor.path} has an invalid cpu.max policy`
            );
        }
        if (
            ancestor.cpuQuotaMicros !== "max" &&
            ...
            throw new Error(
                `SSE memory evidence ancestor ${ancestor.path} cpu.max is stricter than ${policy.cpuQuotaPercent}%`
            );
        }
    }
}
...
/**
 * Requires the current cgroup to match the reviewed evidence policy exactly.
...
export function assertCgroupResourcePolicy(
    ...
    if (
        ...
        throw new Error(
            `SSE memory evidence requires cpu.max=${policy.cpuQuotaPercent}%; observed ${String(cpuQuota)} ${snapshot.cpuPeriodMicros}`
        );
    }
    if (!snapshot.oomGroup) {
        throw new Error(`SSE memory evidence requires OOMPolicy=${policy.oomPolicy}`);
    }
