...
CREATE TABLE `job_disable_intents` (
	...
	CONSTRAINT "job_disable_intents_end_check" CHECK(("ended_at" IS NULL AND "ended_by_kind" IS NULL AND "ended_by_id" IS NULL AND "ended_reason" IS NULL) OR ("ended_at" IS NOT NULL AND "ended_at" BETWEEN 0 AND 8640000000000000 AND "ended_at" >= "created_at" AND "ended_by_kind" IS NOT NULL AND "ended_by_id" IS NOT NULL AND "ended_reason" IN ('expired', 're-enabled', 'replaced') AND (("ended_by_kind" = 'user' AND length("ended_by_id") = 36 AND instr("ended_by_id", char(0)) = 0 AND length(replace("ended_by_id", '-', '')) = 32 AND replace("ended_by_id", '-', '') NOT GLOB '*[^0-9a-f]*' AND substr("ended_by_id", 9, 1) = '-' AND substr("ended_by_id", 14, 1) = '-' AND substr("ended_by_id", 15, 1) = '7' AND substr("ended_by_id", 19, 1) = '-' AND substr("ended_by_id", 20, 1) GLOB '[89ab]' AND substr("ended_by_id", 24, 1) = '-') OR ("ended_by_kind" = 'automation' AND length("ended_by_id") BETWEEN 1 AND 64 AND instr("ended_by_id", char(0)) = 0 AND "ended_by_id" = lower("ended_by_id") AND substr("ended_by_id", 1, 1) GLOB '[a-z0-9]' AND "ended_by_id" NOT GLOB '*[^a-z0-9._-]*') OR ("ended_by_kind" = 'system' AND length("ended_by_id") BETWEEN 1 AND 128 AND instr("ended_by_id", char(0)) = 0 AND "ended_by_id" = lower("ended_by_id") AND substr("ended_by_id", 1, 1) GLOB '[a-z0-9]' AND "ended_by_id" NOT GLOB '*[^a-z0-9._-]*')) AND ("ended_reason" <> 'expired' OR ("ended_by_kind" = 'system' AND "expires_at" IS NOT NULL AND "ended_at" >= "expires_at")))),
...
CREATE TABLE `job_runs` (
	...
	CONSTRAINT "job_runs_time_check" CHECK("queued_at" BETWEEN 0 AND 8640000000000000 AND "updated_at" BETWEEN 0 AND 8640000000000000 AND "updated_at" >= "queued_at" AND ("first_started_at" IS NULL OR ("first_started_at" BETWEEN 0 AND 8640000000000000 AND "first_started_at" >= "queued_at")) AND ("last_attempt_started_at" IS NULL OR ("last_attempt_started_at" BETWEEN 0 AND 8640000000000000 AND "last_attempt_started_at" >= "first_started_at")) AND ("finished_at" IS NULL OR ("finished_at" BETWEEN 0 AND 8640000000000000 AND "finished_at" >= COALESCE("last_attempt_started_at", "queued_at"))))
) STRICT, WITHOUT ROWID;
--> statement-breakpoint
...
CREATE TABLE `scheduled_jobs` (
	...
	CONSTRAINT "scheduled_jobs_time_zone_check" CHECK("time_zone" IS NULL OR (length("time_zone") BETWEEN 1 AND 64 AND instr("time_zone", char(0)) = 0)),
...
CREATE TABLE `worker_instances` (
	...
	CONSTRAINT "worker_instances_release_id_check" CHECK(length("release_id") = 40 AND instr("release_id", char(0)) = 0 AND "release_id" = lower("release_id") AND "release_id" NOT GLOB '*[^0-9a-f]*'),
