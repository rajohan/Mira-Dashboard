...
describe("source-boundary import target validation", () => {
    ...
    test("rejects repository escapes from browser and integration tests", async () => {
        const projectRoot = await temporaryProject();
        try {
            await writeFile(
                path.join(projectRoot, "src", "browser", "escape.test.ts"),
                'import "../../../outside.ts";'
            );
            await writeFile(
                path.join(projectRoot, "src", "test", "integration", "escape.ts"),
                'import "../../../../outside.ts";'
            );
            ...
            expect(
                violations
                    .filter((violation) => violation.message.includes("may not escape"))
                    .map(({ importer }) => importer)
            ).toEqual(["src/browser/escape.test.ts", "src/test/integration/escape.ts"]);
        } finally {
            await rm(projectRoot, { force: true, recursive: true });
        }
    });
