...
- Production-composition tests cover the reviewed root through ticket issuance and one-shot raw
  ...
  rows are now implemented. The current inventory is **107 implemented, 48 planned, and two
  reviewed removals** out of 157 legacy endpoints; browser routes remain 12 implemented and four
  planned. `GET /api/media` remains planned until opaque transcript-bound local-history media is
  implemented; this slice does not claim database backup/restore parity.
