...
| 5 — Privileged and external domains | Started                              | Files and Logs have closed reviewed `/files` and `/logs` parity. Moltbook has its claim-fenced worker snapshot and `/moltbook`; Terminal is a worker-owned bounded PTY; `/settings` owns bounded OpenClaw configuration/export/restart; and Overview Service Actions expose six fixed intents. The read-only `database.overview` procedure and `/database` workflow are implemented, with privileged backup/restore still separate. The full Docker vertical is implemented: source-derived Engine/Compose inventory, cache/LKG, fixed worker operations, updater/history/notifications, exact-byte Compose image replacement, exact-path Git commit/push, protected two-operation worker broker, and `/docker`; all eighteen Docker endpoints are implemented, with the three console routes mapped to the existing bounded Terminal lifecycle. GitHub, deployment, database backup/restore, production host provisioning, and the remaining privileged adapters stay open, so the Phase 5 exit gate is not claimed. |
...
- The living parity inventory is now **131 implemented, 23 planned, and three reviewed removals** out
  ...
  closed in the greenfield implementation; production activation remains separate, and this slice
  did not edit the live `/opt/docker` source, restart containers, deploy, commit, or push.
