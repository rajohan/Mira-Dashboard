...
Install the exact Bun version in `.bun-version`, clone the repository, and enter the
self-contained `greenfield/` root before cutover (the repository root after cutover). Then run:
