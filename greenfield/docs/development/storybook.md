...
return success. Only exact diagnostics from the pinned Headless UI and dnd-kit versions are filtered
for the specific stories that exercise those upstream paths; the same message from any other story,
or any other browser error, remains visible and fatal to the package command.
...
All Storybook packages are pinned together at `10.6.0-alpha.4`. That release contains the
