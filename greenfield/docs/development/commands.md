...
| `bun run test coverage [bun\|browser\|storybook\|merge]`                     | Run/merge the shared coverage plan                                                                          | Writes `coverage/`; installs Chromium only when needed              |
...
Lefthook is the single Git-hook manager. Bootstrap installs its pinned `pre-commit` and `pre-push`
hooks, and configuration validation is part of `bun run check`. Pre-commit runs fast format and
